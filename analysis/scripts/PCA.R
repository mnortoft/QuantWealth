...
pre_TOT$TOT <- rowSums(pre_TOT[3:20])
...
grave.pca <- PCA(imp_data$completeObs, ncp = 5, quanti.sup = c(9:14), quali.sup = c(1,16), graph = FALSE)
#plot(grave.pca, choix = "ind", habillage = c("ageGroup", "SexGender")) #only works with one qualitative variable
summary(grave.pca)
...
# PCA_paper_group <- fviz_pca_biplot(grave.pca,
#                                  # Individuals
#                                  geom.ind = "point",
