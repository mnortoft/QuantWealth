...
## Flint artefacts
Knapping and grinding times for flint artefacts (mostly axes) were collected from @hansen_madsen83; @madsen84; Olausson [-@olausson83a: table 3], Olausson [-@olausson83b: 62], uncut YouTube videos of flint axe manufacture by the highly experienced flint knapper Will Lord [-@lord19; @lord17; @lord20], and one YouTube comment (written 2013) by Wulf Hein on his own experimental video of axe grinding (see reference data tables for YouTube URLs), as well as from an interview with flint knapper and all-round prehistoric crafts specialist Morten Kutschera. 
...
#load flint reference person-hour calculations:
source(here('analysis/scripts/PH_flint.R'), echo = TRUE, encoding = "UTF-8")

gridExtra::grid.arrange(knapp_plot, thinButt_grindPlot, box_all, ncol = 3, top = grid::textGrob("Grinding and knapping time (in PH) for different methods and sources",gp=grid::gpar(fontsize=6,font=3)), widths=c(2, 5, 4))
```

<br>

The reference data on knapping a flint axe from both the experimental literature and the informant had a median of `r thin_butt_knap_median` and a standard deviation of `r thin_butt_knap_sd`, making it fairly reliable. 
Axe length has also been noted ethnographically as a marker of prestige [@olausson83b: 12-14]. The larger surface of longer axes might in theory also affect the person-hours needed for grinding. This may also be the case for the reference data. However, this could also be due to the increased person-hours noted for weighted (2-3 persons) experiments and ethnographically (often reported in days making conversion to hours very uncertain). If the focus is only total hours of grinding (i.e. irrespective of the persons involved in the weighted experiments), there is no clear increase (in this data) in grinding time with increased length, except perhaps for the longest axes (29-31 cm). Again, the ethnographic example is less reliable because it is reported in days (the mean of 5-8 hour work days used here), and the knapping of this axe was done by direct percussion which would require extra grinding time according to the author [@vial40: 160; via @madsen84: 57-58]. The lack of a clear relationship between length and grinding time could be due to grinding the axe in longer movements across the grinding surface, in which case time may rely more on the grinding skill keeping as much surface contact between the axe and the grindstone as possible in each movement. 

```{r Flint-axe-length, fig.cap="Scatterplots comparing thin-butted flint axe length vs. grinding hours: left: person-hours, right: total labour-hours (for weighted grinding by multiple people).", results='markup', fig.show='asis'}
...
If we remove the two less reliable ethnographic examples of grinding, we get a median of `r thinButt_grind_median`. The degree of polish, however, may also be a factor, in the sense that extraordinarily polished axes would have taken more time, or required more skill, e.g. for axes that are extraordinarily long and thin, with one example being 27-30 cm long and only 0.8-1.5 cm thick as is noted for special ceremonial axes by [@olausson83b: 12]. Ceremonial axes with special and exotic (e.g. varicoloured) stone quality may also have added extra value. Therefore, it may still be justified using length of above 27 cm as a factor of value along with lower thickness, extra polish/shine, and rare or variegated colours. If each of these adds, e.g. 10% to the raw person-hour estimate, this could potentially add 40% value. To this could be added estimated travel-hours from the quarry, which may have been very distant. 
...
<br>= `r flint_raw_1` kg/person (over 6.5 months) = `r flint_raw_2` kg/person/month = `r flint_raw_3` kg/person/day = `r flint_raw_4` kg/person/hour. 
...
<br><br>
Taking an example of a typical thin-butted flint axe quarried 14 travel hours away, the formula for flint axes would be as follows: 
<br>axe blank `r axe_blank` PH (quarrying and blank knapping) + 
<br>knapping `r thin_butt_knap_median` PH +
<br>grinding `r thinButt_grind_median` PH +
...
<br>length >30 cm (+50% to knapping and grinding PH) `r flint_long` + 
...
<br>= `r round(flint_axeAdze_total_PH + flint_long + flint_axe_thin + flint_shine + flint_colour, 2)` total score. To this we may add extra long distance to the raw material, exclusivity ("prestige" measure) and scarcity in the given community, 

Based on the calculations above, the raw person-hours for making hafted flint tools such as axes and daggers can thus be summarised as follows (raw material for flint daggers assumed to be half of axes, here without travel hours, scarcity, "prestige", and extraordinary features): 
...
### Scarcity of flint
Scarcity of imported flints is based on the numbers in Kolar 2018: total Corded Ware graves known: 413/total count of Corded Ware flint types by origin (@kolar18: table 52). Number of graves with each flint type were not available in this work, so the total count has been used instead. Scarcity for local and indeterminable cherts is based on the total number of flint objects in Corded Ware (255) in @kolar18: table 51. The total number of flint/silicite axes (18) is based on @kolar18: table 55.

<br>

#### Flint axe estimated manufacture time in archaeological grave data

The total value points for flint axes, taken from the reference data and applied to the archaeological data, is given in table \@ref(tab:flint-axe-arch).
```{r flint-axe-arch}
...
If we compare this with the knapping time given by the informant (MK), we can see the similarity for most of these (table \@ref(tab:inf-flintknap-time).
...
# PCA
To include grave depth in the PCA, some missing values were imputed using the missMDA package. Figures \@ref(fig:impute-precision1) and \@ref(fig:impute-precision2) show the precision of the predicted imputed grave depth (from 100 generated predicted datasets) compared to the observed grave depth, and the impact of the imputation on the PCA. The variation of the imputed values in the plots shows the possible variability of the prediction using a parametric bootsrap method which is preferable for evaluating uncertainty in PCA. 
...
A principal components analysis (PCA) was carried out on the different wealth and value measures with materials overlayed. PCA of dimensions 1 and 2 can be seen in the main paper, while PCA of dimensions 2 and 3 is visualized here with two clusters highlighting the meat cluster on dimension 2, see figure \@ref(fig:pca-dim-2-3). 

```{r pca-dim-2-3, fig.cap="Biplot of PCA on dimensions 2 and 3.", results='asis', out.width="100%"}
PCA_paper_2_3
```
...
# Acknowledgements
I wish to sincerely thank the crafts people that endured my endless questioning on time estimates for endless numbers of objects at different production stages, in this regard especially Morten Kutschera, Inger Heebøl at Sagnlandet Lejre and Pauline Ferrisse, and Torben Johansen (referred to as the informants "MK", "IH", "PF", and "TJ") who have been crucial sources of manufacturing time reference data and craft knowledge. Their help, expertise and deep knowledge has been extremely valuable to me. I also wish to thank my supervisor Rune Iversen for his feedback and support, and Torben Johansen with whom I will keep carrying out metal forging experiments, and Johanna Brinkmann for fruitful discussions on metal technology and her metal person-hour framework which has been in part incorporated in this study, Also thanks to Roeland Paardekooper of EXARC for helping me make connections with some experimental archaeologists for follow-up questions. Finally, I deeply thank Solveig Chaudesaigues-Clausen for everyday sparring on experimental archaeology and Stone Age technology, and the Saxo Institute for providing me the funding for the PhD project that this paper is a part of. 
...
Sources not mentioned in this text but used in in the reference tables are given here (see main reference list below for full reference details): 
@hein_lund12
